...
def plot_dual_delta_p_with_theoretical_fit(
    df, target_id, body_part="Delta_P_Left Chest", level=None, angle=None, save=False
):
    """
    ...
    """
    df_id = df[df["PCS_ID"] == target_id]
    available_levels = df_id["PCS_Level"].dropna().unique().tolist()
    if level is None:
        print(
            f"[INFO] Available Level values for PCS ID {target_id}: {available_levels}"
        )
        return
    if level not in available_levels:
        ...
        return
    df_level = df_id[df_id["PCS_Level"] == level]
    a = "PCS_Level"
    df_level = df_id[df_id[a] == level]
